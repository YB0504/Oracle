...
-- 3. user04 계정에 role 부여 : connect, resource 롤 부여
grant CONNECT, RESOURCE to user04;

-- 4. user04 계정으로 접속 후 테이블 생성 (cmd창에서 생성)
...
-- 2. 생성된 롤에 grant로 시스템 권한을 추가
grant create session, create table, create view to mrole;

-- 3. 새로운 계정(user05)을 생성하여 role 부여
create user user05
identified by tiger;
